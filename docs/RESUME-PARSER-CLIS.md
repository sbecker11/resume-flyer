...
## Parse CLI (resume-to-flyer)

- **Contract / docs:** Resume-parser README and package; no separate contract file in contracts/.
- **Console command:** `resume-to-flyer <docx-path> -o <output-dir>` (see [resume-parser README](https://github.com/sbecker11/resume-parser#usage)).
- **Module invocation:** `python3 -m resume_parser.resume_to_flyer <docx-path> -o <output-dir>` (override via `RESUME_PARSER_MODULE`).
