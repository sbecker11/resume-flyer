...
import {
  ...
  FLOCK_Z_MAX,
  FLOCK_BIZCARD_Z_INDEX_MIN,
  FLOCK_BIZCARD_Z_INDEX_MAX,
  FLOCK_SKILL_Z_INDEX_MIN,
  FLOCK_SKILL_Z_INDEX_MAX,
  Z_from_z_index,
  z_index_from_Z,
  z_from_z_index,
  z_index_from_z,
...
describe('zUtils', () => {
  ...
  describe('Z = maxZ - z_index mapping', () => {
    it('Z_from_z_index gives distance (higher z_index = lower Z)', () => {
      expect(Z_from_z_index(1)).toBe(FLOCK_Z_MAX - 1);
      expect(Z_from_z_index(FLOCK_Z_MAX)).toBe(0);
    });
  ...
  describe('bizCard vs skillCard z_index invariant', () => {
    it('biz z_index max is less than skill z_index min so skill cards render above biz cards', () => {
      expect(FLOCK_BIZCARD_Z_INDEX_MAX).toBeLessThan(FLOCK_SKILL_Z_INDEX_MIN);
    });
