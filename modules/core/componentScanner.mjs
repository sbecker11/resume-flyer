/**
 * Component Scanner - Modular IM compliance scanner
 * Refactored from 1,411-line monolithic scanner into focused modules
 * 
 * This is the main entry point that orchestrates the modular scanning system.
 * The heavy lifting is done by specialized detector modules.
 */

import { ScannerConfig } from './scanning/config/ScannerConfig.mjs';
import { CodeParser } from './scanning/analysis/CodeParser.mjs';
import { DOMViolationDetector } from './scanning/violations/DOMViolationDetector.mjs';
import { ImportDependencyDetector } from './scanning/violations/ImportDependencyDetector.mjs';
import { CompliancePatternDetector } from './scanning/violations/CompliancePatternDetector.mjs';
import { MigrationAutoFixer } from './scanning/migration/MigrationAutoFixer.mjs';
import { ReportGenerator } from './scanning/reporting/ReportGenerator.mjs';
...
export class ComponentScanner {
    constructor(configPath = null) {
        ...
        this.autoFixer = new MigrationAutoFixer(this.codeParser);

        // Initialize violation detectors
        this.detectors = {
            dom: new DOMViolationDetector(this.codeParser),
            imports: new ImportDependencyDetector(this.codeParser),
            compliance: new CompliancePatternDetector(this.codeParser)
        };
    ...
    /**
     * Main scan entry point - replaces the old scanProject method
     */
    async scanProject(rootPath = '.') {
        console.log('🔍 Starting IM Compliance Scan...');
        ...
        try {
            ...
            // 2. Scan files for violations
            const scanResults = await this.scanFiles(files);

            // 3. Apply auto-fixes if enabled
            if (this.config.getAutoFixConfig().enabled) {
                await this.applyAutoFixes(scanResults);
            }

            // 4. Generate compliance report
            const report = this.reportGenerator.generateComplianceReport(scanResults);

            // 5. Output results
            this.outputResults(report, scanResults);
    ...
    async discoverFiles(rootPath) {
        ...
        try {
            await this._scanDirectory(rootPath, files);
        } catch (error) {
            console.warn(`⚠️ Could not scan directory ${rootPath}:`, error.message);
        }
        
        return files.filter(file => this.config.shouldScanFile(file.path));
    }
    ...
    _shouldSkipDirectory(name) {
        const skipDirs = ['node_modules', '.git', 'dist', 'build', '.nuxt', '.output', 'archived_components', 'tmp'];
    ...
    _isComponentFile(filename) {
        ...
        const excludePatterns = ['BaseComponent', 'Abstract'];
